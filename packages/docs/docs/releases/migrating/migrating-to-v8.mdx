...
Most of the options below drive enterprise features, which live in the
`dockview-enterprise` package and need a licence key. On the free `dockview`
package those options are inert (the console logs a note naming the package to
...
install and apply a key, and [Licensing](/docs/overview/licence) for the full
