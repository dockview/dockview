...
// 'blocked' (popup blocker), 'url-refused' (the same-origin guard),
// 'unscriptable' (the host returned a window the opener cannot reach into) or
// 'closed' (it went away before loading).
api.onDidOpenPopoutWindowFail((failure: PopoutWindowFailure) => {
    console.warn('popout failed:', failure.reason, failure.error);
});

// Fires as Dockview lets go of a popout window, while that window is still
// open. Covers every popout the component opened - `addPopoutGroup`, the group
// context menu, a restored layout - and every reason for closing one, component
// disposal included.
api.onWillClosePopoutWindow((event: PopoutWindowEvent) => {
    // event.window is still live here; event.id is the window's target name,
    // as passed to `window.open`
});
```
