...
`extraWindowFeatures` appends entries to the features string, for example a marker an Electron `setWindowOpenHandler` can match on (booleans serialize as `1`/`0`):
