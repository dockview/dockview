...
This matters most for drags that cross a window boundary. Only an HTML5 drag rides an OS drag session, so only an HTML5 drag can move a tab between a popout window and the main window. An embedded webview reporting a coarse pointer and no fine pointer resolves `'auto'` to the pointer backend, where no host configuration will make that drag work. Check `html5` rather than assuming it.
